...
export class HyperliquidService {
  ...
  constructor() {
    ...
    if (config.privateKey && config.userWallet) {
  ...
  private validateOrderSize(size: number, coin: string, price: number): { size: number; formattedSize: string } {
    const decimals = this.getSizeDecimals(coin)
    ...
    if (orderValue < config.minOrderValue) {
      const minSize = config.minOrderValue / price
  ...
  private async placeMarketBuy(coin: string, size: number, fillPrice: number, reduceOnly: boolean = false): Promise<OrderResponse> {
    if (!this.walletClient) throw new Error('Wallet client not initialized')

    const coinIndex = this.getCoinIndex(coin)
    const validated = this.validateOrderSize(size, coin, fillPrice)
  ...
  private async placeMarketSell(coin: string, size: number, fillPrice: number, reduceOnly: boolean = false): Promise<OrderResponse> {
    if (!this.walletClient) throw new Error('Wallet client not initialized')
  ...
  async openLong(coin: string, size: number, price: number): Promise<OrderResponse> {
    return await this.placeMarketBuy(coin, size, price)
  }

  async openShort(coin: string, size: number, price: number): Promise<OrderResponse> {
    return await this.placeMarketSell(coin, size, price)
  }

  async closePosition(coin: string, price: number, size?: number): Promise<OrderResponse> {
    if (!this.walletClient) throw new Error('Wallet client not initialized')

    const positions = await this.getOpenPositions(config.userWallet)
    ...
    if (position.side === 'long') {
      return await this.placeMarketSell(coin, closeSize, price, true)
    } else {
      return await this.placeMarketBuy(coin, closeSize, price, true)
    }
  }

  async reducePosition(coin: string, reduceSize: number, price: number): Promise<OrderResponse> {
    return await this.closePosition(coin, price, reduceSize)
  }

  async addToPosition(coin: string, size: number, price: number, side: 'long' | 'short'): Promise<OrderResponse> {
    if (side === 'long') {
      return await this.placeMarketBuy(coin, size, price)
    } else {
      return await this.placeMarketSell(coin, size, price)
